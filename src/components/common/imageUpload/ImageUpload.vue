            ...
            this.$nextTick(() => {
                ...
                if (this.canChangeX) {
                    if (this.changeCropTypeX === 1) {
                        ...
                        if (this.cropOldW + fw > 0) {
                            this.cropW =
                                this.cropOldW + fw + this.cropOffsertX <=
                                wrapperW
                                    ? this.cropOldW + fw
                ...
                if (this.canChangeY) {
                    if (this.changeCropTypeY === 1) {
                        ...
                        if (this.cropOldH + fh > 0) {
                            this.cropH =
                                this.cropOldH + fh + this.cropOffsertY <=
                                wrapperH
                                    ? this.cropOldH + fh
        ...
        getImgAxis(x, y, scale) {
            ...
            switch (this.rotate) {
                ...
                case -3:
                    obj.x1 =
                        x +
                        (this.trueWidth * (1 - scale)) / 2 +
                        (imgW - imgH) / 2;
                    obj.x2 = obj.x1 + this.trueHeight * scale;
                    obj.y1 =
                        y +
                        (this.trueHeight * (1 - scale)) / 2 +
                        (imgH - imgW) / 2;
      ...
      <div class="show-preview" style="width: ${obj.w}px; height: ${
        ...
        reload() {
            ...
            img.onload = () => {
                ...
                // 得到外层容器的宽度高度
                this.w = parseFloat(
                    window.getComputedStyle(this.$refs.cropper).width
                );
                this.h = parseFloat(
                    window.getComputedStyle(this.$refs.cropper).height
                );
                // 存入图片真实高度
        ...
        goAutoCrop(cw, ch) {
            ...
            if (this.centerBox) {
                ...
                let imgW =
                    (switchWH ? this.trueHeight : this.trueWidth) * this.scale;
                let imgH =
                    (switchWH ? this.trueWidth : this.trueHeight) * this.scale;
